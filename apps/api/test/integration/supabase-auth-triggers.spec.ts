...
describe('Supabase Auth Triggers E2E @Integration', () => {
    ...
    afterAll(async () => {
        ...
        for (const uid of createdAuthUserIds) {
            ...
            try {
                await pool.query(
                    'DELETE FROM public.parents WHERE auth_user_id = $1',
                    [uid],
                );
            } catch {
                /* best-effort */
    ...
    describe('handle_new_user() trigger', () => {
        ...
        it(
            ...
            when(async () => {
                ...
                expect(result.rows[0].display_name).toBe(
                    email.split('@')[0],
                );
            }),
        );
    ...
    describe('custom_access_token_hook()', () => {
        it(
            ...
            when(async () => {
                const signin = await gotruePost(
                    '/token?grant_type=password',
                    { email: primaryEmail, password: testPassword },
                );
